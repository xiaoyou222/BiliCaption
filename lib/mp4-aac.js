const BiliCaptionMp4 = (() => {
  ...
  function resolveChunkLimits(options = {}) {
    ...
    return {
      // 这里的默认值是 BiliCaption 自己的安全上限。服务商限制更严时
      // 可以往下收紧，但不允许调用方把分片放大到默认上限之外。
      maxBytes: Number.isFinite(requestedBytes) && requestedBytes > 64 * 1024
        ? Math.min(CHUNK_BYTES, requestedBytes)
        : CHUNK_BYTES,
      maxSeconds: Number.isFinite(requestedSeconds) && requestedSeconds > 1
        ? Math.min(CHUNK_SECONDS, requestedSeconds)
        : CHUNK_SECONDS
    };
  }
  ...
  function buildAdtsChunks(buffer, track, options = {}) {
    const { maxBytes, maxSeconds } = resolveChunkLimits(options);
    ...
    while (startIndex < track.samples.length) {
      ...
      while (endIndex < track.samples.length) {
        ...
        if (endIndex > startIndex && (nextBytes > maxBytes || nextDur > maxSeconds)) break;
  ...
  function splitFragmented(buffer, view, boxes, options = {}) {
    ...
    while (i < fragments.length) {
      ...
      while (i < fragments.length) {
        ...
        if (i > chunkStartIndex && (size + pieceSize > maxBytes || piece.start - start > maxSeconds)) break;
  ...
  async function* iterateFmp4Chunks(reader, options = {}) {
    ...
    const cutFrames = (count) => {
      ...
      const mediaDur = taken.length * step;
      const start = mediaClock;
      const end = start + mediaDur;
      const hasNext = frames.length > 0;
      const chunk = {
      ...
      let back = 0;
      let i = taken.length;
      while (hasNext && i > 1 && back < OVERLAP_SECONDS) {
        i -= 1;
        back += step;
        frames.unshift(taken[i]);
      }
      mediaClock = end - back;
      emitted += 1;
      return chunk;
    ...
    const maybeChunk = () => {
      if (useAdts) {
        ...
        while (i < frames.length) {
          ...
          const nextDur = mediaDur + frameSeconds();
          if (i > 0 && (nextSize > maxBytes || nextDur > maxSeconds)) break;
          size = nextSize;
          mediaDur = nextDur;
          i += 1;
        }
        if (i <= 0) return null;
        if (i >= frames.length && mediaDur < maxSeconds && size < maxBytes) return null;
        return cutFrames(i);
      }
      ...
      if (i >= frags.length && mediaDur < maxSeconds && size < maxBytes) return null;
      return takeChunk(i, frags[i]?.start ?? frags[i - 1].end);
    };
    ...
    const parseAvailable = function* () {
      ...
      for (;;) {
        ...
        if (box.type === "ftyp" || box.type === "moov") {
          ...
          const bytes = buf.slice(box.offset, box.dataEnd);
          const localView = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
          const localMoof = readBoxes(localView, 0, bytes.byteLength)[0] || null;
          pendingMoof = {
            bytes,
            start: localMoof ? parseTfdt(localView, localMoof) / timescale : 0
          };
        } else if (box.type === "mdat" && pendingMoof) {
          let start = 0;
          let bytes;
          try {
            start = pendingMoof.start || 0;
            bytes = concatBytes([pendingMoof.bytes, buf.slice(box.offset, box.dataEnd)]);
          ...
          fallback = true;
          fallbackBlob = new Blob([...initParts, buf.slice(box.offset, box.dataEnd)], { type: "audio/mp4" });
          parsed = box.dataEnd;
          return;
        }
        parsed = box.dataEnd;
    ...
    if (fallback) {
      yield { fallback: true, blob: fallbackBlob || new Blob(initParts, { type: "audio/mp4" }) };
      return;
    }
    if (useAdts && frames.length) {
      ...
      const chunk = takeChunk(frags.length, null);
      if (chunk) yield chunk;
    } else if (initBlob) {
      yield { fallback: true, blob: initBlob };
    }
  }
