...
(function (global) {
  ...
  async function readPage(page, trackUrl = '', loadedUrl = '') {
    ...
    if (yt) {
      ...
      const tracks = (data.captions?.playerCaptionsTracklistRenderer?.captionTracks || []).map((t) => ({
      ...
      if (loadedUrl) {
        ...
        if (loaded.protocol === 'https:' && loaded.hostname === url.hostname && loaded.pathname === '/api/timedtext' &&
            ['v', 'lang', 'kind'].every((key) => loaded.searchParams.get(key) === url.searchParams.get(key)) && !loaded.searchParams.has('tlang')) {
